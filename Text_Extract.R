...
# input_txt <- file.path(input_dir,files) # this creates a path to each letter
...
text_extract <- function(letter) {
  ...
  write(bod, paste0(config_extract_folderpath, "/", id, ".txt"))
}

# To run:
# result <- lapply(input, text_extract)

text_extract_fromdb <- function(letter) {
  
  id <- letter[c("publicIdentifier")]
  body <- letter[c("body")]
  
  write(body, paste0(config_extract_folderpath, "/", id, ".txt"))
}
